...
void delete_text(int id)
{
	

	// Check if the array is empty or if i is out of bounds
	if (num_render_strings == 0 || id < 0 || id >= num_render_strings) {
		printf("error in delete_text\n");
		return;
	}

	// Copy the value from the last cell to the i-th cell
	renderable_texts[id] = renderable_texts[num_render_strings - 1];
	num_render_strings--;
}
...
void changeSize_text(int id, float sizeChange, const char* temp)
{
	//printf("renderable_texts[id].size_px before:%d\n", renderable_texts[id].size_px);
	text_to_vbo(temp,
		(renderable_texts[id].size_px + sizeChange),
	...
	renderable_texts[id].size_px = (renderable_texts[id].size_px + sizeChange);
//	printf("renderable_texts[id].size_px after:%d\n", renderable_texts[id].size_px);
...
int Redo_Text(int id, const char* str, GLFWwindow* window) {
	// just re-generate the existing VBOs and point count
	double xpos, ypos;
...
void draw_texts () {
	...
	int loc = glGetUniformLocation(font_sp, "tex");
	//printf("loc:%d\n", loc);
...
glm::vec2 TextgetPos(int id)
{
	return glm::vec2(renderable_texts[id].tl_x - 0.01125f, renderable_texts[id].tl_y-0.05125f);
}